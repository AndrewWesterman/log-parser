...
int main(int argc, char** argv)
{
  ...
  while( getline(inFile, line)){
	...
	if ( Word_Info.get_address().compare(data1) == 0 ) {
	  ...
	  /* Since each line of the command contains 32 bits, which is 2 words
	  ...
	  while(index < num_words){
	    ...
	    /* set the time for the rest of the words except the last one*/
	    previous_command->set_time(Word_Info.get_time());
   ...
   //------------------------------------------------------------------------------------------------------------------
	...
	/* Compare the address with "40000C20". If they are equal, then the word start at 0 to N */
